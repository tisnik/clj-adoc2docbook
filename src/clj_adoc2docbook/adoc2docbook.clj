...
(defn asciidoc-like-transformation
  ...
  ; TODO - just a quick hack, would be better to replace with proper state
  ; machine
  (-> string
...
(defn list-item
  ...
  (let [star-index (inc (.indexOf line "*"))
    ...
    (str "    <listitem><para>"
         (asciidoc-like-transformation link-to-bugzilla content)
         "</para></listitem>\n")))
...
(defn screen-notrim
  [line]
  (cond (.startsWith line "    ") (subs line 4)
...
(defn create-normal-links
  ...
  (let [href (re-find http-regexp string)]
    (if href
      (str/replace string
                   http-regexp
                   #(str "<ulink url='" (first %1) "'>" (first %1) "</ulink>"))
      string)))
...
(defn create-bugzilla-links
  ...
  (let [bz-href (re-find bz-href-regexp string)]
    (if bz-href
      (str/replace string
                   bz-href-regexp
                   #(str "BZ#<ulink url='" link-to-bugzilla (second %1) "'>" (second %1) "</ulink>"))
      string)))
